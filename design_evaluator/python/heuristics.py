...
__all__ = ['findLineSegIntersection', 'findOrientation', 'feasibility', 'stability', 'orientation', 'triangles',  'density', 'threeStars']
...
# given their endpoints as inputs
	if (findOrientation(p1,q1,p2) != findOrientation(p1,q1,q2)) & (findOrientation(p2,q2,p1) != findOrientation(p2,q2,q1)):
		if np.all(np.isclose(p1,p2)) | np.all(np.isclose(q1,q2)) | np.all(np.isclose(p1,q2)) | np.all(np.isclose(q1,p2)):
			intersect = False; 
		else:
			intersect = True;
	else:
